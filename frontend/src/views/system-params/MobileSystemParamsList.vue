...
async function fetchList() {
  loading.value = true; try {
    const res = await paramApi.getParamList({ keyword: keyword.value || undefined, pageIndex: pageIndex.value, pageSize: 10 }); const items = res.data.items ?? []; if (pageIndex.value === 1)
...
/** 跳转到详情页 */
function goDetail(id: string) { router.push(`/m/system-params/${id}`) }
onMounted(fetchList)

const showForm = ref(false); const form = reactive({ code: '', value: '', category: '', description: '' }); const submitting = ref(false)
/** 打开 Create */
...
<template>
  <div class="m-page">
    <van-sticky><van-search v-model="keyword" placeholder="搜索编码/名称" shape="round" @search="onSearch" @clear="onSearch" /></van-sticky>
    <div class="m-toolbar">
      ...
      <div class="m-card-list">
        <div v-for="item in list" :key="item.id" class="m-card-list__item" @click="goDetail(item.id)">
          <div class="card-header">
            <span class="card-header__title">{{ item.code }} <van-icon name="arrow" size="14" color="var(--color-text-dim)" /></span>
          </div>
          <div class="card-row">
            <span class="card-row__label">值</span><span>{{ item.value }}</span>
          </div>
    ...
    <van-action-sheet v-model:show="showForm" title="新增参数">
      <div style="padding:16px">
        <van-field v-model="form.code" label="编码" /><van-field v-model="form.value" label="值" /><van-field v-model="form.category" label="分类" /><van-field v-model="form.description" label="描述" type="textarea" autosize /><van-button round block type="primary" :loading="submitting" style="margin-top:16px" @click="handleCreate">
          确定
        </van-button>
      </div>
    </van-action-sheet>
