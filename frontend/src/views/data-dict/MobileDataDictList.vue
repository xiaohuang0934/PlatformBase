...
<template>
  <div class="m-page">
      ...
      <div class="m-card-list">
        <div v-for="item in list" :key="item.id" class="m-card-list__item" @click="goDetail(item.id)">
          <div class="card-header">
            <span class="card-header__title">{{ item.typeName }} <van-icon name="arrow" size="14" color="var(--color-text-dim)" /></span>
          </div>
          <div class="card-row">
            <span class="card-row__label">编码</span><span>{{ item.typeCode }}</span>
          </div>
    ...
    <van-action-sheet v-model:show="showForm" title="新增类型">
      <div style="padding:16px">
        <van-field v-model="form.typeName" label="名称" /><van-field v-model="form.typeCode" label="编码" /><van-field v-model="form.description" label="描述" type="textarea" autosize /><van-button round block type="primary" :loading="submitting" style="margin-top:16px" @click="handleCreate">
          确定
        </van-button>
      </div>
    </van-action-sheet>
